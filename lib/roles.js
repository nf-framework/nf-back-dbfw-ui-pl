...
export async function addRole(context) {
    const res = await dbapi.broker('nfc.roles.mod', {
...
export async function getRole(context) {
    const data = await dbapi.query(`
        SELECT id, code, caption FROM nfc.v4roles where id = :roleId;
    `, { roleId: context.body.args.roleId }, { context, returnFirst: true })
